...
{
  # Multiple input files can be separated by commas
  dataFilePathsSplit = strsplit(dataFilePath, ",")[[1]]
  ...
  for (x in dataFilePaths)
  {
    ...
    dataList[[x]] = as.matrix(read.table(x, sep="\t", stringsAsFactors=F, header=T, row.names=1, check.names=F, quote="\""))
  }
